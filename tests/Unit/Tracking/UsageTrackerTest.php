...
namespace Soderlind\AiValve\Tests\Unit\Tracking;
...
final class UsageTrackerTest extends TestCase {
	...
	public function test_record_increments_four_counters(): void {
		...
		$this->assertSame( 600, $updated[ 'soderlind_aivalve_tokens_daily_' . $today . '_test-plugin' ] );
		$this->assertSame( 600, $updated[ 'soderlind_aivalve_tokens_monthly_' . $month . '_test-plugin' ] );
		$this->assertSame( 600, $updated[ 'soderlind_aivalve_tokens_daily_' . $today . '_*' ] );
		$this->assertSame( 600, $updated[ 'soderlind_aivalve_tokens_monthly_' . $month . '_*' ] );
	}
	...
	public function test_record_increments_provider_counters_when_given(): void {
		...
		$this->assertSame( 600, $updated[ 'soderlind_aivalve_tokens_daily_' . $today . '_*' ] );
		$this->assertSame( 600, $updated[ 'soderlind_aivalve_tokens_monthly_' . $month . '_*' ] );
		$this->assertSame( 600, $updated[ 'soderlind_aivalve_tokens_daily_' . $today . '_provider:openai' ] );
		$this->assertSame( 600, $updated[ 'soderlind_aivalve_tokens_monthly_' . $month . '_provider:openai' ] );
	}
	...
	public function test_record_skips_provider_counters_when_empty(): void {
		...
		$this->assertArrayNotHasKey( 'soderlind_aivalve_tokens_daily_' . $today . '_provider:', $updated );
	}
	...
	public function test_plugin_tokens_today_reads_correct_key(): void {
		$today = gmdate( 'Y-m-d' );
		$expected_key = 'soderlind_aivalve_tokens_daily_' . $today . '_my-plugin';
	...
	public function test_plugin_tokens_this_month_reads_correct_key(): void {
		$month = gmdate( 'Y-m' );
		$expected_key = 'soderlind_aivalve_tokens_monthly_' . $month . '_my-plugin';
	...
	public function test_global_tokens_today(): void {
		$today = gmdate( 'Y-m-d' );
		$expected_key = 'soderlind_aivalve_tokens_daily_' . $today . '_*';
	...
	public function test_global_tokens_this_month(): void {
		$month = gmdate( 'Y-m' );
		$expected_key = 'soderlind_aivalve_tokens_monthly_' . $month . '_*';
	...
	public function test_provider_tokens_today_reads_correct_key(): void {
		$today = gmdate( 'Y-m-d' );
		$expected_key = 'soderlind_aivalve_tokens_daily_' . $today . '_provider:openai';
	...
	public function test_provider_tokens_this_month_reads_correct_key(): void {
		$month = gmdate( 'Y-m' );
		$expected_key = 'soderlind_aivalve_tokens_monthly_' . $month . '_provider:anthropic';
	...
	public function test_global_daily_pct_with_limit(): void {
		$today = gmdate( 'Y-m-d' );
		$global_key = 'soderlind_aivalve_tokens_daily_' . $today . '_*';
